...
${indent}${s}` : "\n";
  ...
  var YAMLMap = class extends Collection {
    ...
    toJSON(_, ctx, Type) {
      const map2 = Type ? new Type() : ctx && ctx.mapAsMap ? new Map() : {};
      if (ctx && ctx.onCreate)
        ctx.onCreate(map2);
...
${cn.comment}` : item.comment;
  ...
  var set2 = {
    ...
    createNode(schema4, iterable, ctx) {
      const { replacer } = ctx;
      const set3 = new YAMLSet(schema4);
      if (iterable && Symbol.iterator in Object(iterable))
...
${end.comment}` : end.comment;
  ...
  // src/worker/dedicatedWorker/utils.ts
  var u = __toModule(require_dist());
  var baseConfigUrl = `https://public.aitmed.com/config/`;
  function getOrFetch() {
    return __async(this, arguments, function* (key = "", _a = {}) {
      var _b = _a, {
        baseUrl = "",
      ...
      try {
        if (key.startsWith("http")) {
          return (_b2 = (_a2 = yield fetch(key))[type]) == null ? void 0 : _b2.call(_a2);
        } else {
          if (isWrappedUrl(key)) {
            if (key.startsWith("config:")) {
              let configName = unwrapUrl(key);
              let config = yield db.get(configName);
              let configVersion = version || getVersionFromConfigObject(deviceType, env, config);
              let fetchConfig = (configName2) => __async(this, null, function* () {
                try {
                  const resp = yield fetch(`${baseConfigUrl}${configName2}.yml`);
                  const configYml = yield resp.text();
                  config = parse(configYml);
                  yield db.set("config", config);
                  return config;
                } catch (error) {
              ...
              return (_d = (_c = yield fetch(`${baseUrl}${name}.yml`, rest)) == null ? void 0 : _c[type]) == null ? void 0 : _d.call(_c);
            } else if (key.startsWith("page:")) {
              let name = unwrapUrl(key);
              name.endsWith(".yml") && name.replace(".yml", "");
              return (_f = (_e = yield fetch(`${baseUrl}${name}.yml`, rest)) == null ? void 0 : _e[type]) == null ? void 0 : _f.call(_e);
            }
          }
          return (_h = (_g = yield fetch(key, rest)) == null ? void 0 : _g[type]) == null ? void 0 : _h.call(_g);
        }
      } catch (error) {
        if (error instanceof Error)
          throw error;
  ...
  function getVersionFromConfigObject(deviceType = "web", env = "test", config) {
    var _a, _b;
    return u.isObj(config) ? ((_b = (_a = config == null ? void 0 : config[deviceType]) == null ? void 0 : _a.cadlVersion) == null ? void 0 : _b[env]) || "" : null;
  }
  function isWrappedUrl(url = "") {
  ...
  // src/worker/dedicatedWorker/commands.ts
  var Commands = class {
    constructor() {
      this.commands = {};
    }
    createCommand(command2, fn) {
      const commandFn = (options, opts) => __async(this, null, function* () {
        try {
          return fn == null ? void 0 : fn(options, opts);
        } catch (error) {
          if (error instanceof Error)
            throw error;
          throw new Error(String(error));
  ...
  commands.createCommand(command.CACHE_GET, function(options) {
    return __async(this, null, function* () {
      const { key } = options || {};
      try {
      } catch (error) {
  ...
  // src/worker/dedicatedWorker/dedicatedWorker.ts
  var style = "color:#157DEC;font-weight:400;";
  var tag = `%c[dedicatedWorker]`;
  var log = console.log.bind(console);
  function createDedicatedWorker(options) {
    let { commands: commandsConfig = [] } = options || {};
    for (const commandConfig of commandsConfig) {
      commands_default.createCommand(commandConfig.command, commandConfig.fn);
    }
    self.addEventListener("message", function onWorkerMessage(msg) {
      return __async(this, null, function* () {
        var _a, _b;
        let { command: command2, options: options2 } = msg.data || {};
        log(`${tag} Message`, style, msg);
        if (command2) {
          this.postMessage({
            command: command2,
            result: yield (_b = (_a = commands_default.commands)[command2]) == null ? void 0 : _b.call(_a, options2, {
    ...
    self.addEventListener("rejectionhandled", function(rejEvt) {
      return __async(this, null, function* () {
        log(`${tag} Promise rejection handled`, style, {
          promise: yield rejEvt.promise,
    ...
    self.addEventListener("unhandledrejection", function(unRejEvt) {
      return __async(this, null, function* () {
        log(`${tag} Promise rejection unhandled`, style, {
          eventPhase: unRejEvt.eventPhase,
          promise: yield unRejEvt.promise,
    ...
    const api = {
      ...
      getOrFetch,
      getStore: () => __async(this, null, function* () {
        if (!(yield get(`noodl`)))
          yield set(`noodl`, {});
        return get("noodl");
      })
  ...
  var dedicatedWorker = createDedicatedWorker();
  var dedicatedWorker_default = dedicatedWorker;
})();
