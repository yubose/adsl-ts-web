...
connection.on(JsStore.EVENT.Create, function() {
  console.log(`%c[piWorker.ts] Create`, `color:${_color}`, arguments[0]);
});
connection.on(JsStore.EVENT.Open, function() {
  console.log(`%c[piWorker.ts] Open`, `color:${_color}`, arguments[0]);
});
connection.on(JsStore.EVENT.RequestQueueEmpty, function() {
  console.log(`%c[piWorker.ts] RequestQueueEmpty`, `color:${_color}`, arguments[0]);
});
connection.on(JsStore.EVENT.RequestQueueFilled, function() {
  console.log(`%c[piWorker.ts] RequestQueueFilled`, `color:${_color}`, arguments[0]);
});
connection.on(JsStore.EVENT.Upgrade, function() {
  console.log(`%c[piWorker.ts] Upgrade`, `color:${_color}`, arguments[0]);
});
connection.initDb({
  name: "noodl",
  version: 2,
  ...
  const res = await pi.runSql(`SELECT * from CPT`);
  console.log(`[query] SELECT * from CPT`, res);
  pi.use({
    all(evtName, ...args) {
      console.log(`%c[piWorker.ts] ${evtName}`, `color:${_color};`, args);
    },
    async message(evt) {
      var _a, _b;
      const data = evt.data;
      const type = data == null ? void 0 : data.type;
      console.log(`%c[lib] Message "${type}"`, `color:${_color};`, data);
      switch (type) {
        case "storeData": {
    ...
    messageError(evt) {
      console.log(`%c[lib] messageError`, `color: ${_color}`, evt);
    },
    rejectionHandled(evt) {
      console.log(`%c[lib] rejectionHandled`, `color: ${_color}`, evt);
    },
    rejectionUnhandled(evt) {
      console.log(`%c[lib] rejectionUnhandled`, `color: ${_color}`, evt);
    }
  });
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("e0ec80321c8d747d3998")
/******/ })();
/******/ 
/******/ }
);
//# sourceMappingURL=main.a518a5370dcbfb20f102.hot-update.js.map