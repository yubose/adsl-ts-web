...
function isSameNodeType<V>(
  type: string,
  v: unknown,
): v is V extends infer P ? P : any

function isSameNodeType<V>(
  type: string,
): (v: unknown) => v is V extends infer P ? P : any

function isSameNodeType<V>(type: string, v1?: unknown) {
  if (v1) {
    return !!(v1 && typeof v1 === 'object') && v1['__ntype'] === type
  }
  return (v: unknown): v is V extends infer P ? P : any =>
...
const is = {
  ...
  stringNode: isSameNodeType<NoodlString<any>>(nkey.string),
