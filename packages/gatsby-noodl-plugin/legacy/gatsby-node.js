...
exports.sourceNodes = async (args, pluginOptions) => {
  ...
  createNodeIdFn = createNodeId
  createContentDigestFn = createContentDigest
    ...
    on: {
      createComponent(comp, opts) {
        const context = {
          ...opts,
          page: opts?.page?.page || '',
          parent: u.omit(opts.parent?.toJSON?.() || {}, ['children']) || null,
        }

        if (!data.componentsByPage[opts.page.page]) {
          data.componentsByPage[opts.page.page] = {}
        }
        data.componentsByPage[opts.page.page][comp.id] = {
          context,
          blueprint: u.omit(comp.blueprint, ['children']),
        ...
        //   parentId: comp.parent?.id,
        //   context,
        //   id: createNodeId(comp.id),
        //   children: [],
        //   internal: {
        //     contentDigest: createContentDigest(comp.toString()),
        //     type: 'NoodlComponent',
        //   },
        // })
        traverse(comp.blueprint, opts.path || [])
      },
      patch: u.reduce(
  ...
  for (const [name, pageObject] of u.entries(data.parsedPages)) {
    const serializedPageObject = JSON.stringify(pageObject)
    createNode({
      name,
      slug: `/${name}/`,
      content: serializedPageObject,
      isPreload: false,
      id: createNodeId(`noodl_${name}`),
      children: [],
      internal: {
        contentDigest: createContentDigest(serializedPageObject),
        type: NOODL_PAGE_NODE_TYPE,
      },
    })
    data.pages.app[name] = pageObject
  }

  // Loads/parses noodl yml pages into GraphQL nodes
  // await Promise.all(
  ...
  //       const pagesKey = preload.includes(name) ? 'preload' : 'app'
...
exports.createPages = async function createPages(args, pluginOptions) {
  try {
    ...
    if (errors) {
      ...
      fs.writeJson(
        path.join(__dirname, './pages-output.json'),
        data.parsedPages,
        {
          spaces: 2,
        },
      ...
      await Promise.all(
        u.entries(data.parsedPages).map(async ([pageName, pageObject]) => {
          if (pageName in data.pages.app) {
            pageObject.components = await generateComponents(
              pageObject.components,
            )

            const stringifiedPageObject = JSON.stringify(pageObject)
            const slug = `/${pageName}/`
            ...
            createPage({
              ...
              context: {
                pageName,
                pageObject: JSON.parse(stringifiedPageObject),
                componentMap: data.componentsByPage[pageName],
                slug,
                isPreload: false,
              },
            })
