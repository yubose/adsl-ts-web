...
export function ensureArr(arr: any): any[] {
  if (isArr(arr)) {
    return arr
...
export function jarowinkler(
  ...
  for (mi = 0; mi < min.length; mi++) {
    c1 = min[mi]
    for (
      xi = Math.max(mi - range, 0), xn = Math.min(mi + range + 1, max.length);
      xi < xn;
      xi++
    ) {
 ...
 * the two provided arrays.
 */
export function tanimoto(a: string | string[], b: string | string[]): number {
  a = ensureArr(a)
  b = ensureArr(b)
