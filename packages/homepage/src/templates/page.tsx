...
import { RenderComponentCallbackArgs } from '@/pages'
...
function NoodlPageTemplate(props: NoodlPageTemplateProps) {
  ...
  const [state, setState] = React.useState(() => {
    const st = {}
    for (const component of pageObject.components) {
  ...
  const renderComponent = React.useCallback(
    (
      ...
      const _children = u.reduce(
        u.array(nchildren),
        (acc, component) => {
          return component ? acc.concat(renderComponent(component)) : acc
        },
        [],
      )
  ...
  return (
    <Layout>
      <Seo />
      {pageObject.components?.map?.((c) => renderComponent(c)) || null}
    </Layout>
  )
}
