...
export class Toast {
    ...
    constructor(message: Message, options: ToastOptions = {}) {
        const { timeout = 0, action, type = 'default', cancel, groupId = 'toast-container', groupStyle, toastNumber = 3 } = options
    ...
    setContainer(): void {
        container = document.querySelector(`.${this.options.groupId}`) as HTMLDivElement
        if (!container) {
            container = document.createElement('div')
            container.className = `${this.options.groupId}`
            if(this.options.groupStyle){
                for(const [key,value] of Object.entries(this.options.groupStyle)){
                    container.style.setProperty(key,value)
                }
            }
            // const appContainer = document.querySelector('#root')
            // appContainer && appContainer.appendChild(container)
...
function sortToast(): void {
    ...
    toasts.forEach((toast, index) => {
        ...
        if (sortIndex > 1 && sortIndex < 3) {
            ...
            el.style.setProperty('transform','translate3d(0,calc(var(--hover-offset-y) - var(--stack-gap) * (var(--index) - 1)),0)')
        }
        else {
